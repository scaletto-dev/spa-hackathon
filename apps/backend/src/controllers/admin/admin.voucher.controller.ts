...
export class AdminVoucherController {
    ...
    async createVoucher(
        ...
        try {
            const { code, title, description, discountType, discountValue, minPurchaseAmount, maxDiscountAmount, usageLimit, validFrom, validUntil, isActive } = req.body;

            // Validation
            if (!code || !title || !discountType || discountValue === undefined) {
                throw new ValidationError('Missing required fields: code, title, discountType, discountValue');
            }
    ...
    async updateVoucher(
        ...
        try {
            ...
            if (processedData.maxDiscountAmount !== undefined) {
                processedData.maxDiscountAmount = processedData.maxDiscountAmount ? Number(processedData.maxDiscountAmount) : null;
            }
            if (processedData.usageLimit !== undefined) {
                processedData.usageLimit = processedData.usageLimit ? Number(processedData.usageLimit) : null;
            }
