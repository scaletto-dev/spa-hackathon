...
const services = [{
  ...
  description: 'Advanced AI technology analyzes your skin and creates a personalized treatment plan for optimal results',
  duration: '60 min',
  price: '$150'
}, {
  id: 2,
    ...
    duration: 0.5
      ...
      <div className="mb-8">
        ...
        <p className="text-gray-600">
          Choose from our range of AI-enhanced beauty treatments
        </p>
      </div>
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
        {services.map(service => <motion.div key={service.id} whileHover={{
        y: -5
      }} onClick={() => handleSelectService(service)} className={`cursor-pointer bg-white/70 backdrop-blur-xl rounded-3xl border-2 ${bookingData.service?.id === service.id ? 'border-pink-500 shadow-lg shadow-pink-200' : 'border-white/50 shadow-lg'} overflow-hidden transition-all`}>
            <div className="relative h-44 overflow-hidden">
              ...
              {bookingData.service?.id === service.id && <div className="absolute top-4 right-4 w-8 h-8 bg-pink-500 rounded-full flex items-center justify-center">
      ...
      <div className="flex justify-end mt-12">
        ...
        scale: 0.95
      }} onClick={onNext} disabled={!bookingData.service} className={`flex items-center gap-2 px-8 py-4 rounded-full font-semibold shadow-xl ${bookingData.service ? 'bg-gradient-to-r from-pink-500 to-purple-500 text-white' : 'bg-gray-200 text-gray-400 cursor-not-allowed'}`}>
          Continue
