...
export function CustomDropdown({
    ...
    return (
        <div className="relative" ref={dropdownRef}>
            ...
            {isOpen && (
                <div
                    className={`absolute top-full left-0 right-0 mt-2 bg-white rounded-xl border-2 ${colors.menu} shadow-xl overflow-hidden z-50 animate-in fade-in slide-in-from-top-2 duration-200`}
                >
                    {options.map((option) => (
                        <button
                            key={option.value}
