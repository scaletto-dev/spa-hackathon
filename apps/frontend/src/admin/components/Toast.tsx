...
export function Toast({
  ...
  return <div className="fixed top-4 right-4 z-[60] animate-in slide-in-from-top-2 fade-in">
      <div className="bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-pink-100 p-4 flex items-center gap-3 min-w-[300px]">
        <div className={`w-10 h-10 rounded-full bg-gradient-to-r ${colors[type]} flex items-center justify-center flex-shrink-0`}>
          <Icon className="w-5 h-5 text-white" />
        </div>
        <p className="text-sm text-gray-800 flex-1">{message}</p>
        <button onClick={onClose} className="p-1 hover:bg-pink-50 rounded-full transition-colors">
