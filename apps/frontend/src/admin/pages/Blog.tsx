...
export function Blog() {
    ...
    const handleTogglePublish = async (postId: string, currentStatus: string) => {
        try {
            ...
            setToast({
                message: `Post ${currentStatus === 'PUBLISHED' ? 'unpublished' : 'published'}!`,
                type: 'success',
            });
            fetch();
        } catch (err: any) {
            ...
            <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-2xl p-4 border border-purple-100">
                <div className="flex items-center gap-2">
                    ...
                    <p className="text-sm text-gray-700">
                        <span className="font-semibold">Blog Posts:</span> {posts?.length || 0}{' '}
                        total posts
                    </p>
                    <span className="ml-auto px-3 py-1 bg-white rounded-full text-xs font-medium text-purple-600">
                        {posts?.filter((p: any) => p.publishedAt).length || 0} Published
                    </span>
                </div>
            </div>
