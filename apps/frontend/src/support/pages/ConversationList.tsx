    ...
    useEffect(() => {
        ...
        socket.on(
            ...
            (newConversation: {
                ...
                const mappedConv: Conversation = {
                    ...
                    status: (newConversation.status?.toLowerCase() || 'pending') as
                        | 'pending'
                        | 'active'
                        | 'closed',
            ...
            (data: { conversationId: string; lastMessage: string; timestamp: string }) => {
                setConversations((prev) => {
                    ...
                    if (exists) {
                        ...
                        (async () => {
                            try {
                                const newConv = await supportApi.getConversation(
                                    data.conversationId
                                );
                                const mappedConv: Conversation = {
                                    id: newConv.id,
                                    ...
                                    status: newConv.status.toLowerCase() as
                                        | 'pending'
                                        | 'active'
                                        | 'closed',
                                ...
                                setConversations((p) => {
                                    ...
                                    console.log(
                                        '✅ Adding new conversation to list:',
                                        data.conversationId
                                    );
                                    return [mappedConv, ...p];
                                });
        ...
        socket.on('conversation:assigned', (data: { conversationId: string; staffId: string }) => {
            setConversations((prev) =>
                prev.map((conv) =>
                    conv.id === data.conversationId
                        ? { ...conv, status: 'active', assignedTo: data.staffId }
                        : conv
                )
            );
        });
            ...
            setConversations((prev) =>
                prev.map((conv) =>
                    conv.id === conversationId
                        ? { ...conv, assignedTo: 'me', status: 'active' }
                        : conv
                )
            );
                ...
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-4">
                    ...
                    >
                        ...
                        <span className="text-xs md:text-sm font-medium text-gray-700">
                            {isConnected
                                ? t('support.chat.connected')
                                : t('support.chat.disconnected')}
                        </span>
                    </motion.div>
                </div>
                    ...
                    {isLoading ? (
                        ...
                        filteredConversations.map((conversation, index) => (
                            <motion.div
                                ...
                                whileTap={{ scale: 0.99 }}
                                onClick={() =>
                                    navigate(`/support-dashboard/chat/${conversation.id}`)
                                }
                            ...
                            >
                                <div className="flex items-start justify-between gap-3">
                                    ...
                                    <button
                                        onClick={(e) =>
                                            toggleSelectConversation(conversation.id, e)
                                        }
                                    ...
                                    <div className="flex items-start gap-3 flex-1 min-w-0">
                                        ...
                                        <div className="flex-1 min-w-0">
                                            <div className="flex flex-wrap items-center gap-2 mb-1.5">
                                                ...
                                                >
                                                    {t(
                                                        `support.dashboard.stats.${conversation.status}`
                                                    )}
                                                </motion.span>
                                            </div>
                        ...
                        >
                            ...
                            <p className="text-gray-500 font-medium">
                                {t('support.dashboard.noConversations')}
                            </p>
                            <p className="text-sm text-gray-400 mt-1">
                                {t('support.dashboard.noConversationsDesc')}
                            </p>
                        </motion.div>
                    )}
                </div>
