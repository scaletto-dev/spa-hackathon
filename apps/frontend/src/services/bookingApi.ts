...
export const getVNPayPaymentStatus = async (bookingId: string): Promise<any> => {
    try {
        const response = await axiosInstance.get(`${API_BASE}/payments/vnpay/status/${bookingId}`);
        return response.data.data;
    } catch (error) {
        console.error('Failed to get VNPay payment status:', error);
        throw error;
    }
};
